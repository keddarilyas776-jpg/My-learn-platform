...
export default function CourseCard({ course, onSubscribe }: Props) {
  ...
  const getEmbedUrl = (url: string) => {
    // دعم قوائم التشغيل كما في قاعدة بياناتك
    ...
    <>
      <div className="bg-white rounded-3xl shadow-sm border border-neutral-100 overflow-hidden flex flex-col h-full hover:shadow-xl transition-all duration-300" dir="rtl">
        ...
        <div className="p-6 flex flex-col flex-1 text-right">
          ...
          <button 
            onClick={handleStart} 
            className={`w-full ${colors.btn} text-white text-sm font-bold py-4 rounded-2xl shadow-lg flex items-center justify-center gap-2`}
          >
      ...
      {showModal && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/90 p-2 overflow-y-auto backdrop-blur-sm">
          <div className="bg-white w-full max-w-4xl rounded-[2.5rem] overflow-hidden relative my-auto border border-white/20">
            
            <div className="p-5 flex justify-between items-center border-b bg-white sticky top-0 z-10">
              <button onClick={() => setShowModal(false)} className="bg-neutral-100 p-2 rounded-full"><X size={20} /></button>
            ...
            <div className="p-6 bg-neutral-50 text-right space-y-6" dir="rtl">
              <div className="bg-white p-5 rounded-3xl border border-neutral-100 shadow-sm flex items-center justify-between">
                ...
                <div className="bg-amber-50 p-3 rounded-2xl"><Award size={24} className="text-amber-500" /></div>
              </div>

              <div className="bg-white p-6 rounded-3xl border border-neutral-100 shadow-sm">
                <h4 className="text-sm font-black text-neutral-800 mb-4">ما تقييمك لهذا المحتوى؟</h4>
                <div className="flex gap-2 mb-5">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <button key={s} onClick={() => setUserRating(s)} className={`text-3xl ${userRating >= s ? 'text-amber-400' : 'text-neutral-200'}`}>★</button>
                  ))}
                </div>
                <textarea 
                  className="w-full border-2 border-neutral-50 rounded-2xl p-4 text-xs mb-4 bg-neutral-50"
                  placeholder="أخبرنا عن تجربتك.."
                  rows={3}
                  value={comment}
                  onChange={(e) => setComment(e.target.value)}
                />
                <button onClick={submitReview} disabled={isSubmitting} className="w-full bg-neutral-900 text-white py-4 rounded-2xl font-bold text-xs">
